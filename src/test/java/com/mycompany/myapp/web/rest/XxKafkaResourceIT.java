...
class XxKafkaResourceIT {
    ...
    void setup() {
        ...
        XxKafkaResource kafkaResource = new XxKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/xx-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/xx-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
